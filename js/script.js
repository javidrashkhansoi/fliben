...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/header.js


const headerObservers = new HeaderObservers();

;// CONCATENATED MODULE: ./src/js/scripts/scripts/search-form.js
...
if (searchForm && searchButton && closeButton) {
  if (headerLinks) {
    ...
    searchForm.style.setProperty("--right", searchFormRight);
  }
...
const descriptionTitle = description?.querySelector(".description__title");

if (descriptionTitle) {
  const move = new Move({
    destinationSelector: ".description",
    targetSelector: ".description .description__title",
    index: "first",
  });
}

const applicationSection = document.querySelector(".application");
const applicationSectionSliderButtons = applicationSection?.querySelector(".slider-buttons");

if (applicationSectionSliderButtons) {
  const move = new Move({
    destinationSelector: ".application-slider",
    targetSelector: ".application .slider-buttons",
  });
}

const videogallerySection = document.querySelector(".videogallery");
const videogallerySectionSliderButtons = videogallerySection?.querySelector(".slider-buttons");

if (videogallerySectionSliderButtons) {
  const move = new Move({
    destinationSelector: ".videogallery-slider",
    targetSelector: ".videogallery .slider-buttons",
  });
}

const systemLinesBreadcrumbs = document.querySelector(".system-header__breadcrumbs");
const systemCategoriesContainer = document.querySelector(".categories__container");

if (systemLinesBreadcrumbs && systemCategoriesContainer) {
  const move = new Move({
    destinationSelector: ".categories__container",
    targetSelector: ".system-header__breadcrumbs",
    index: "first",
  });
}
...
const youtubeThumbnails = ["maxresdefault", "hq720", "sddefault", "sd3", "sd2", "sd1", "hqdefault", "hqdefault", "hq3", "hq2", "hq1", "0", "mqdefault", "mq3", "mq2", "mq1"];
const youtubeThumbnailsLength = youtubeThumbnails.length;
const storage = localStorage;

if (!storage.getItem("media")) storage.setItem("media", "{}");
...
class Media {
  ...
  constructor(click) {
    ...
    if (mediaBlocks.length) {
      this.#hosting = [...new Set([...mediaBlocks].map(/** @param {HTMLDivElement} mediaBlock */ mediaBlock => {
        const mediaDataset = mediaBlock.dataset;

        let { hosting, media: id } = mediaDataset;

        if (hosting) hosting = hosting.trim();
        if (id) id = id.trim();

        if (!id) {
          hosting = hostingNames.none
        } else if (!hosting) {
          hosting = this.#setHosting(id);
        }

        mediaDataset.hosting = hosting
  ...
  #setHosting(id) {
    ...
    switch (true) {
      case /^[a-zA-Z0-9_-]{11}$/.test(id):
        hosting = hostingNames.youtube;
        break;
      case /^[0-9]{9}$/.test(id):
        hosting = hostingNames.vimeo;
        break;
      default:
        hostingNames.youtube;
    }
  ...
  #createMediaPreview() {
    const mediaStorage = JSON.parse(storage.getItem("media"));

    mediaBlocks.forEach(mediaBlock => {
      const mediaDataset = mediaBlock.dataset;
      const { media: id } = mediaDataset;

      /** @type {HTMLButtonElement} */
      const button = mediaBlock.querySelector("[data-play], button, a");

      if (id && button) {
        const { hosting } = mediaDataset;

        if (!mediaStorage[hosting]) mediaStorage[hosting] = {};
        if (!this.#apiStatus[hosting]) this.#apiStatus[hosting] = false;

        const block = document.createElement("div");

        block.id = `${hostingNames[hosting]}-${Date.now().toString(36)}-${Math.random().toString(36).substring(2)}`;

        /** @type {HTMLPictureElement | HTMLImageElement} */
        let picture = mediaBlock.querySelector("img, picture");

        if (!picture) {
          const mediaStorageHostingObject = mediaStorage[hosting][id];

          if (hosting === hostingNames.youtube) {
            const source = document.createElement("source");
            const img = document.createElement("img");

            picture = document.createElement("picture");

            if (mediaStorageHostingObject) {
              source.srcset = mediaStorageHostingObject.webp;
              img.src = mediaStorageHostingObject.jpg;
            } else {
              this.#getImageSrc(source, id, hosting, "webp");
              this.#getImageSrc(img, id, hosting);
            }
          ...
          } else if (hosting === hostingNames.vimeo) {
            picture = document.createElement("img");
            mediaStorageHostingObject ?
              picture.src = mediaStorageHostingObject.jpg :
              this.#getImageSrc(picture, id, hosting);
            picture.alt = "";
          }

          mediaBlock.insertAdjacentElement("afterbegin", picture);
        }

        mediaBlock.insertAdjacentElement("afterbegin", block);

        button.addEventListener("click", () => {
          if (!this.#apiStatus[hosting]) {
            const script = document.createElement("script");
            const firstScriptTag = document.getElementsByTagName("script")[0];

            script.src = apiLinks[hosting];
            firstScriptTag.parentNode.insertBefore(script, firstScriptTag);
            this.#apiStatus[hosting] = true;
    ...
    storage.setItem("media", JSON.stringify(mediaStorage));
  }

  /**
   * @param {HTMLImageElement | HTMLSourceElement} image
   * @param {string} id
   * @param {"jpg" | "webp"} type
   * @param {number} index
   * @returns {void | undefined}
   */
  #getImageSrc(image, id, hosting = hostingNames.youtube, type = "jpg", index = 0) {
    if (index >= youtubeThumbnailsLength) return;

    const img = document.createElement("img")
    const thumbnailName = youtubeThumbnails[index];

    let src;

    if (hosting === hostingNames.youtube) {
      src = `https://i.ytimg.com/vi${type === "webp" ? `_${type}` : ""}/${id}/${thumbnailName}.${type}`;
    } else if (hosting === hostingNames.vimeo) {
      src = `https://vumbnail.com/${id}.${type}`;
    }
    ...
    img.addEventListener("load", () => {
      ...
      if (hosting === hostingNames.youtube && width === 120 && height === 90) {
        this.#getImageSrc(image, id, hosting, type, ++index);
      } else {
        const mediaStorage = JSON.parse(storage.getItem("media"));

        let mediaStorageHostingObject = mediaStorage[hosting][id];

        if (!mediaStorageHostingObject) mediaStorageHostingObject = mediaStorage[hosting][id] = {};

        if (image instanceof HTMLImageElement) {
          image.src = src;
          mediaStorageHostingObject.jpg = src;
        } else if (image instanceof HTMLSourceElement) {
          image.srcset = src;
          mediaStorageHostingObject.webp = src;
        }
  ...
  /**
   * @param {HTMLDivElement} mediaBlock
   */
  #callApi(mediaBlock) {
    const { dataset, classList } = mediaBlock;
    const { media, hosting } = dataset
    const image = mediaBlock.querySelector("picture, img");
    const button = mediaBlock.querySelector("[data-play], button, a");
    const block = mediaBlock.querySelector(`div[id^="${hosting}"]`);

    this.click(mediaBlock);
    image.remove();
    button.remove();
    classList.add(addedClass);
    this.#createIframe(block, media, hosting);
  }
...
const media = new Media((block) => {
  const caption = block.querySelector(".card-caption");

  caption?.remove();
});

;// CONCATENATED MODULE: ./src/js/scripts/scripts/show-hidden-text.js
/** @type {NodeListOf<HTMLDivElement} */
const texts = document.querySelectorAll(".portfolio-about__text, .text");

texts?.forEach(text => {
  /** @type {HTMLButtonElement} */
  const button = text?.querySelector(".portfolio-about__button, .text__button");
  const paragraphs = text?.querySelectorAll("p");
  /** @type {HTMLParagraphElement} */
  const p = text?.querySelector("p:first-child");

  if (button && p) {
    /** @type {{wordCount: string | number}} */
    let { wordCount } = text.dataset;

    wordCount = wordCount && wordCount.trim() && Number.isInteger(+wordCount) && +wordCount > 0 ? +wordCount : 50;
    ...
    if (hiddenText || paragraphsLength > 1) {
      /** @type {HTMLSpanElement | string} */
      let span = "";

      if (hiddenText) {
        span = document.createElement("span");
        span.classList.add("text__hidden");
        span.innerHTML = hiddenText;
      }

      p.innerHTML = showText;
      p.append(space, span, space, button);

      button.addEventListener("click", () => {
        button.remove();
        p.innerHTML = showText + space + hiddenText;
...
if (heroSlider) {
  const slider = new Swiper(heroSlider, {
    ...
    navigation: {
      prevEl: heroSlider.querySelector(".slider-arrow--prev"),
      nextEl: heroSlider.querySelector(".slider-arrow--next"),
    },
    pagination: {
    ...
    a11y: {
      ...
      slideLabelMessage: "{{index}}-ое изображение из {{slidesLength}}",
    },
    slidesPerView: 1,
  });
}

;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/description.js
...
if (descriptionSlider) {
  const slider = new Swiper(descriptionSlider, {
    ...
    pagination: {
      el: descriptionSlider.querySelector(".slider-pagination"),
      clickable: true,
    },
    keyboard: {
      enabled: true,
...
;// CONCATENATED MODULE: ./src/js/decorators/debounce.js
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/slider-height.js


const slider_height_media = matchMedia("(min-width: 769px)");

function setSliderHeight(slideItem, slider) {
  const addSlideHeight = debounce((slide, target) => {
    if (slider_height_media.matches) {
      const height = Math.ceil(parseFloat(getComputedStyle(slide).blockSize)) * 2 + Math.ceil(parseFloat(getComputedStyle(target.swiper.wrapperEl).gap));
  ...
  const sliderResizeObserver = new ResizeObserver(entries => {
    entries.forEach(entry => {
      const { target } = entry;
      const slide = target.querySelector(slideItem);

      if (slide) {
        addSlideHeight(slide, target);
      }
    });
  });

  if (slider_height_media.matches) sliderResizeObserver.observe(slider);

  slider_height_media.addEventListener("change", event => {
    const { matches } = event;
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/videogallery.js




const videogallery = document.querySelector(".videogallery");
const videogallerySlider = videogallery?.querySelector(".videogallery-slider");

if (videogallerySlider) {
  const slider = new Swiper(videogallerySlider, {
    modules: [Keyboard, Navigation, Pagination, Grid, A11y],
    navigation: {
      prevEl: videogallery.querySelector(".slider-arrow--prev"),
      nextEl: videogallery.querySelector(".slider-arrow--next"),
    },
    pagination: {
      el: videogallery.querySelector(".slider-pagination"),
      clickable: true,
    },
    keyboard: {
      enabled: true,
    ...
    a11y: {
      ...
      prevSlideMessage: "Предыдущий слайд",
      paginationBulletMessage: "Перейти на слайд {{index}}",
      slideLabelMessage: "{{index}}-ое видео из {{slidesLength}}",
    },
    slidesPerView: 2,
    slidesPerGroup: 2,
    spaceBetween: 6,
  ...
  setSliderHeight(".video-card", videogallerySlider);
}

;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/system.js



const systemSliderBlocks = document.querySelectorAll(".system-slider");

systemSliderBlocks?.forEach(systemSliderBlock => {
  const systemSlider = systemSliderBlock.querySelector(".system-slider__item");

  const slider = new Swiper(systemSlider, {
    modules: [Keyboard, Navigation, A11y, Pagination],
    navigation: {
      prevEl: systemSliderBlock.querySelector(".slider-arrow--prev"),
      nextEl: systemSliderBlock.querySelector(".slider-arrow--next"),
    },
    pagination: {
      el: systemSliderBlock.querySelector(".slider-pagination"),
      clickable: true,
    },
    keyboard: {
      enabled: true,
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/application.js




const application = document.querySelector(".application");
const applicationSlider = application?.querySelector(".application-slider");

if (applicationSlider) {
  const slider = new Swiper(applicationSlider, {
    modules: [Keyboard, Navigation, Pagination, Grid, A11y],
    navigation: {
      prevEl: application.querySelector(".slider-arrow--prev"),
      nextEl: application.querySelector(".slider-arrow--next"),
    },
    pagination: {
      el: application.querySelector(".slider-pagination"),
      clickable: true,
    },
    keyboard: {
      enabled: true,
  ...
  setSliderHeight(".system-card", applicationSlider);
}

;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/portfolio.js
...
if (portfolioSlider) {
  ...
  let slider = initSlider(media.matches ? "fade" : "creative");

  media.addEventListener("change", event => {
    const { matches } = event;

    slider.destroy();
    slider = initSlider(matches ? "fade" : "creative");
  });

  function initSlider(effect) {
    const slider = new Swiper(portfolioSlider, {
      modules: [Keyboard, Navigation, Pagination, A11y, EffectCreative, EffectFade],
      navigation: {
        prevEl: portfolioSlider.querySelector(".slider-arrow--prev"),
        nextEl: portfolioSlider.querySelector(".slider-arrow--next"),
      },
      pagination: {
        el: portfolioSlider.querySelector(".slider-pagination"),
        clickable: true,
      },
      keyboard: {
        enabled: true,
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/other.js



const otherSlider = document.querySelector(".other-slider");

if (otherSlider) {
  const media = matchMedia("(max-width: 768px)");

  let slider = initSlider(media.matches ? "creative" : "slide");

  media.addEventListener("change", event => {
    const { matches } = event;

    slider.destroy();
    slider = initSlider(matches ? "creative" : "slide");
  });

  function initSlider(effect) {
    const slider = new Swiper(otherSlider, {
      modules: [Keyboard, Navigation, A11y, Pagination, EffectCreative],
      navigation: {
        prevEl: otherSlider.querySelector(".slider-arrow--prev"),
        nextEl: otherSlider.querySelector(".slider-arrow--next"),
      },
      pagination: {
        el: otherSlider.querySelector(".slider-pagination"),
        clickable: true,
      },
      keyboard: {
        enabled: true,
